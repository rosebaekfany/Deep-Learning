...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]
    x_row = x.reshape(N, -1)  # Reshape to (N, D) where D = d_1 * ... * d_k
...
def affine_backward(dout, cache):
    ...
    N = x.shape[0]
    x_row = x.reshape(N, -1)
    dx = np.dot(dout, w.T).reshape(x.shape)  # Reshape to original shape of x
...
def svm_loss(x, y):
    ...
    ###########################################################################
    # TODO: Compute the SVM loss and its gradient                             #
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ...
    # Compute the margins for each example
